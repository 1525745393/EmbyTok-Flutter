...
## 🔮 未来扩展：FastAPI 中间层（路线图）

当前架构为 **Flutter App ↔ Emby 服务器** 直连，足够稳定运行。`backend/` 目录保留了 FastAPI 项目骨架，未来在以下场景下可逐步扩展中间层：

| 阶段 | 规划功能 | 解决的问题 |
|------|---------|------------|
| **Phase 1** | **统一缓存层**（Redis） | 减少 Emby 服务器请求压力；缩略图、媒体列表本地秒级响应 |
| **Phase 2** | **跨用户收藏与历史同步** | 多设备间同步观看进度与收藏状态；无需依赖 Emby 用户体系 |
| **Phase 3** | **多源聚合**（Emby + Jellyfin + Plex） | 一个 App 浏览多个媒体服务器，统一播放体验 |
| **Phase 4** | **数据统计与分析面板** | 观看时长分布、热门媒体排行、家庭成员使用报告 |
| **Phase 5** | **离线转码缓存** | 低带宽环境下提前转码低码率版本；支持移动端离线下载 |
| **Phase 6** | **AI 内容理解** | 自动生成简介、字幕翻译、智能推荐 |

> 💡 **设计哲学**：中间层永远是**可插拔的增强层**，Flutter App 会始终保留直连 Emby 的能力。启用中间层时只需切换 `EmbytokService` 的 baseURL，无需改动 UI 层。

**技术栈（中间层）**：FastAPI + Pydantic + SQLAlchemy + Redis，部署为 Docker 容器。
