...
void main() {
  group('PageNavigationState.isFeedVisible：Feed Tab 可见性判定', () {
    ...
    test('搜索覆盖层（isOverlayPage=true, currentIndex=search）: 不可见', () {
      // 搜索覆盖层时用户关注覆盖层内容，Feed 视频不应在后台播放，
      // 所以 isOverlayPage=true 时 isFeedVisible=false，会触发暂停。
      const state = PageNavigationState(
    ...
    test('历史覆盖层（isOverlayPage=true, currentIndex=history）: 不可见', () {
      // 历史覆盖层同搜索覆盖层：isOverlayPage=true 时 isFeedVisible=false，
      // 会触发暂停，避免视频在覆盖层下后台播放。
      const state = PageNavigationState(
  ...
  group('applyFeedVisibilityChange：核心 pause/play 决策', () {
    ...
    test('Feed → 搜索覆盖层：isFeedVisible 变为 false → 调用 pause', () {
      // isOverlayPage=true 时 isFeedVisible=false，
      // 所以 Feed→搜索覆盖层 isFeedVisible 从 true→false，
      // applyFeedVisibilityChange 触发 pause，避免视频后台播放。
      stubPlaying(mockController);
    ...
    test('Feed → 历史覆盖层：isFeedVisible 变为 false → 调用 pause', () {
      // 同搜索覆盖层：历史覆盖层 isOverlayPage=true → isFeedVisible=false，
      // Feed→历史覆盖层 isFeedVisible 从 true→false，触发 pause。
      stubPlaying(mockController);
  ...
  test('TC-1: isFeedVisible 在搜索覆盖层(isOverlayPage=true)时返回 false', () {
    const state = PageNavigationState(
      currentIndex: PageIndices.search,
      isOverlayPage: true,
    );
    expect(state.isFeedVisible, isFalse,
        reason: '搜索覆盖层时用户关注覆盖层内容，Feed 视频应暂停，视为不可见');
  });
